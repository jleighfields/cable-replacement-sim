...
1. **`rust-toolchain.toml`** at the repository root, pinning the channel and
   declaring `clippy` and `rustfmt` so a fresh machine gets what the gates
   need. `profile = "minimal"` keeps the download small.
...
2. **All three workflows.** `.github/workflows/test.yml`,
   ...
   **These must change in the same commit as the file, or the change makes
   ...
   now misdescribes what actually compiles. Replace the action with a step
   that honours the file — GitHub runners ship rustup, so `rustup show` inside
   the checkout installs the pinned toolchain and its components — and drop
   the `components:` input, which the file now owns.
...
- `rustup show` inside the checkout names the pinned version, not the default.
